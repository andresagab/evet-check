    ...
    <x-modals.modal-dialog wire:model='open' includeFooter="0">
        ...
        <x-slot:title>
            <div class="flex flex-row items-center">
                <h3 class="flex-grow font-bold text-lg uppercase text-purple-800 dark:text-purple-300 select-none">{{ __("messages.data.actions.$action", ['resource' => __('messages.models.event_attendance.model_name')]) }}</h3>
            ...
            <form wire:submit.prevent='submit' autocomplete="off" novalidate class="w-full">
                ...
                <div class="flex flex-col items-center space-y-6 w-full">
                    ...
                    @if($attendance->id)
                        ...
                        <x-sys.events.attendances.data-card :attendance="$attendance"/>
                    @endif

                    {{-- person form --}}
                    <x-cards.card title="Formulario de Asistencia a evento" :footer="0" color="stone-100">
                        <x-slot:content>
                            {{-- form-card --}}
                            <x-sys.events.attendances.form-card :event="$event"/>
                        </x-slot:content>
                    </x-cards.card>
