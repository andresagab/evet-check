    ...
    <x-modals.modal-dialog wire:model='open' includeFooter="0">
        ...
        <x-slot:title>
            <div class="flex flex-row items-center">
                <h3 class="flex-grow font-bold text-lg uppercase text-purple-800 dark:text-purple-300 select-none">{{ __("messages.data.actions.delete", ['resource' => __('messages.models.role.model_name')]) }}</h3>
        ...
        <x-slot:content>
            ...
            <form wire:submit.prevent='submit' autocomplete="off" novalidate class="w-full">
                ...
                <div class="flex flex-col items-center space-y-6 w-full">
                    ...
                    {{-- if permission have id (when action is edit) --}}
                    @if($permission->id)
                        {{-- role data-card --}}
                        <x-admin.permissions.data-card :permission="$permission"/>
                    ...
                    <div class="w-full inline-flex space-x-2 items-center justify-end mt-5">
                        ...
                        {{-- save button --}}
                        <x-buttons.main-button wire:click="delete" type="button" color="red">Eliminar</x-buttons.main-button>
                    </div>
