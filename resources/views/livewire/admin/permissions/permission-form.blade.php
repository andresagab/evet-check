    ...
    <x-modals.modal-dialog wire:model='open' includeFooter="0">
        ...
        <x-slot:title>
            <div class="flex flex-row items-center">
                <h3 class="flex-grow font-bold text-lg uppercase text-purple-800 dark:text-purple-300 select-none">{{ __("messages.data.actions.$action", ['resource' => 'permission']) }}</h3>
        ...
        <x-slot:content>
            ...
            <form wire:submit.prevent='submit' autocomplete="off" novalidate class="w-full">
                ...
                <div class="flex flex-col items-center space-y-6 w-full">
                    ...
                    @if($permission->id)
                        {{-- permission data-card --}}
                        <x-admin.permissions.data-card :permission="$permission"/>
                    @endif

                    {{-- permission form --}}
                    <x-cards.card title="Formulario de Permiso" :footer="0" color="stone-100">
                        <x-slot:content>
                            {{-- form-card --}}
                            <x-admin.permissions.form-card/>
                        </x-slot:content>
                    </x-cards.card>
