...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4 items-baseline w-full">
    ...
    <x-forms.input-group class="w-full">
        {{-- label --}}
        <x-forms.label value="{{ __('messages.models.event_attendance.event') }}" for="frm.event_id" class="required"/>
        {{-- select --}}
        <x-forms.select wire:model="frm.event_id" required>
            {{-- loop generate option list of people --}}
            @foreach(\App\Models\Sys\Event::query()
                ->where('state', 'OP')
                ->whereNotIn('id', $person->event_attendances()->pluck('event_id'))
                ->orderBy('name')->get() as $item)
                <x-forms.option value="{{ $item->id }}" class="text-gray-700 dark:text-stone-400 font-normal">{{ $item->name }} | {{ $item->year }}</x-forms.option>
            @endforeach
        </x-forms.select>
        {{-- error --}}
